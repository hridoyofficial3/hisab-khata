...
function togglePlanning(){
  ...
  if(opening) closeSiblingCollapsibles(toggleEl); // T10: sibling গ্রুপ (panel-notes) থেকে অন্য খোলা প্যানেল বন্ধ
...
function renderPlans(){
  ...
  list.innerHTML = sorted.map(p=>{
    const pv = Math.max(1, Math.min(3, parseInt(p.priority, 10) || 1));
const stars = '★'.repeat(pv) + '☆'.repeat(3 - pv);
    
    const dateRow = p.date ? '<div>'+L('planTargetDatePrefix')+'<b>'+escapeHtml(p.date)+'</b></div>' : '<div>'+L('planNoDatePrefix')+'</div>';
    ...
    return '<div class="plan-item"><div class="pname"><span>'+escapeHtml(p.item)+'</span><span>'+moneyFmt(p.amount)+'</span></div>' +
      ...
      '<div style="display:flex; gap:8px; margin-top:8px; justify-content:flex-end;">' +
        '<button class="btn outline" style="padding:6px 10px; font-size:12px; width:auto;" onclick="openBuyPlanModal('+Number(p.id)+')">'+L('buyNowBtn')+'</button>' +
        '<button class="pdel" onclick="deletePlan('+Number(p.id)+')">×</button>' +
      '</div></div>';
  }).join('');
}
function renderPlanHistory(){
  ...
  list.innerHTML = sorted.map(p=>'<div class="entry"><div class="left"><span class="tag"><span class="accbadge">'+accLabel(p.boughtAccount)+'</span>'+escapeHtml(p.item)+(p.boughtDate ? ' · ' + escapeHtml(p.boughtDate) : '')+'</span></div><div class="actions"><span class="amt expense">'+moneyFmt(p.amount)+'</span></div></div>').join('');
}
...
function openBuyPlanModal(id){
  ...
  setAccountSelectValue(accSel, plan.account || 'cash');
...
document.getElementById('buyPlanContinueBtn').addEventListener('click', ()=>{
  ...
  if(account === 'savings'){
    ...
    if(gtMoney(amt, accountBalance('savings'))){ toast(L('insufficientSavingsMsg')); return; }
    ...
    if(gtMoney(amt, accountBalance(account))){ toast(L('insufficientBalanceMsg')); return; }
    const rows = confirmRow(L('confItem'), escapeHtml(plan.item)) + confirmRow(L('confAmount'), moneyFmt(amt)) +
