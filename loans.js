...
document.getElementById('loanForm').addEventListener('submit', (e)=>{
  ...
  if(ltype === 'given' && amount > accountBalance(account)){
    updateLoanBtnState();
...
document.getElementById('selfLoanTakeForm').addEventListener('submit', (e)=>{
  ...
  if(amount > accountBalance('savings')){ toast(L('insufficientSavingsMsg')); return; }
...
document.getElementById('depositForm').addEventListener('submit', (e)=>{
  ...
  const date = todayStr();
  if(amount > accountBalance(from)){ toast(L('insufficientBalanceMsg')); return; }
...
document.getElementById('transferForm').addEventListener('submit', (e)=>{
  ...
  if(amount > avail){ updateTransferBtnState(); openAlert(tfmt('transferInsufficientMsg', { amt: moneyFmt(amount - avail) })); return; }
  const rows = confirmRow(L('confAmount'), moneyFmt(amount)) + confirmRow(L('confFrom'), accLabel(from)) +
...
document.getElementById('settleDueContinueBtn').addEventListener('click', ()=>{
  ...
  if(!amt || amt <= 0 || amt > due.amount + 0.01){ toast(L('invalidSettleAmountMsg')); return; }
  if(due.type==='payable' && amt > accountBalance(account)){ toast(L('insufficientBalanceMsg')); return; }
...
function deleteDue(id){
  ...
  const netChange = affected.reduce((s,en)=> s + (en.type==='income'? en.amount : -en.amount), 0);
  let msg = L('dueDeleteConfirm');
  if(affected.length > 0){ msg += '\n\n⚠️ ' + numFmt(affected.length) + 'টি লেনদেন এন্ট্রিও মুছে যাবে।'; }
...
function renderLoanHistoryList(){
  ...
  list.innerHTML = combined.map(h=>{
    ...
    return '<div class="entry '+(isPositive ? 'income-bg' : 'expense-bg')+'" onclick="openLoanHistoryDetail(\''+h.kind+'\', '+h.id+')" role="button" tabindex="0">'+
      '<div class="left"><span class="tag"><span class="accbadge">'+badgeText+'</span>'+(name ? name + ' · ' : '')+(h.sortDate || '')+'</span></div>'+
...
function openLoanHistoryDetail(kind, id){
  ...
  if(kind === 'loan'){
    ...
    rows = (name ? '<div style="font-weight:700; margin-bottom:6px;">'+name+'</div>' : '') +
      ...
      '<div>'+L('histDateLabel')+': <b>'+l.date+'</b></div>' +
      (l.note ? '<div>'+L('histNoteLabel')+': '+escapeHtml(l.note)+'</div>' : '') +
      '<div style="color:var(--ledger-green); font-weight:600; margin-top:6px;">✓ '+settledLabel+(l.settledDate ? ' ('+l.settledDate+')' : '')+'</div>';
  } else {
    ...
    rows = '<div style="font-weight:700; margin-bottom:6px;">'+escapeHtml(d.person)+'</div>' +
      ...
      '<div>'+L('histDateLabel')+': <b>'+d.date+'</b></div>' +
      '<div style="color:var(--ledger-green); font-weight:600; margin-top:6px;">✓ '+L('settledLabel')+(d.settledDate ? ' ('+d.settledDate+')' : '')+'</div>';
  }
  body.innerHTML = rows;
...
function openEntryDetail(id){
  ...
  const rows = confirmRow(L('confType'), L(en.type==='income'?'typeIncome':'typeExpense')) +
    ...
    (en.budgetType ? confirmRow(L('confBudgetType'), L(en.budgetType+'Label')) : '') +
    confirmRow(L('confDate'), en.date) + (en.note ? confirmRow(L('confNote'), escapeHtml(en.note)) : '');
  body.innerHTML = rows;
...
function openSettleLoanModal(id){
  ...
  accSel.value = loan.account || 'cash';
...
document.getElementById('settleLoanContinueBtn').addEventListener('click', ()=>{
  ...
  if(!amt || amt <= 0 || amt > loan.amount + 0.01){ toast(L('invalidSettleAmountMsg')); return; }
  if(loan.type==='taken' && amt > accountBalance(account)){ toast(L('insufficientBalanceMsg')); return; }
...
function renderSelfLoanRepayOptions(){
  ...
  if(pending.length === 0){
    ...
    sel.innerHTML = pending.map(l=> '<option value="'+l.id+'">'+moneyFmt(l.amount)+' — '+l.date+(l.note ? ' · '+escapeHtml(l.note) : '')+'</option>').join('');
...
function repaySelfLoan(id, fromAccount, amt){
  ...
  if(payAmt > availForRepay){ openAlert(tfmt('selfLoanRepayInsufficientMsg', { amt: moneyFmt(payAmt - availForRepay) })); return; }
