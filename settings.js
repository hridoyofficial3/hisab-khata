...
function applyDarkMode(){
  const mode = (settings && settings.darkMode) || 'system';
  let effective;
  if(mode === 'system'){
  ...
  if(meta) meta.content = effective === 'dark' ? '#0a1f1a' : '#1F5C4F';
...
document.addEventListener('keydown', (e)=>{
  ...
  if(!document.querySelector('.modal-overlay.open')){ scrollLockCount = 0; unlockBodyScroll(); }
});

/* কীবোর্ড দিয়ে role="button" আইটেম (এন্ট্রি/লোন/due কার্ড) অ্যাক্টিভেট করা — অ্যাক্সেসিবিলিটি ফিক্স */
document.addEventListener('keydown', (e)=>{
